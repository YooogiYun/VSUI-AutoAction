...
class XiaoHongShuPage(BaseObjectClass) :
	"""
	用于描述 木鱼查询页面 的类
	"""
	...
	def __init__(self, driver) :
		...
		self._driver = driver
		self._url = 'https://www.xiaohongshu.com/explore?m_source=itab'
	...
	def __init_PageElements(self) :
		...
		:return:
		"""
		self.CloseButton = BaseObjectClass(self._driver, (By.CSS_SELECTOR, 'div.icon-btn-wrapper.close-button'), self)
		self.PhoneInputField = BaseObjectClass(
			self._driver, (By.CSS_SELECTOR, 'input[type=text][placeholder="输入手机号"]'), self
			)
		self.PasswordInputField = BaseObjectClass(
			self._driver, (By.CSS_SELECTOR, 'input[type=number][placeholder="输入验证码"]'), self
			)
